...
4. [Custom accounts verification](#custom-accounts-verification)
...
- ### Cloudflare Turnstile
  ...
  <Accordion title='Where is my turnstile secret key?'>
...
[`lobbies.find`](/docs/matchmaker/api/lobbies/find) or [`lobbies.join`](/docs/matchmaker/api/lobbies/join)
...
or [`lobbies.join`](/docs/matchmaker/api/lobbies/join) request with the captcha response:
...
1. ### None
   ...
   `find_config` or `join_config` aren't provided, but to explicitly denote this type of verification you can
   add this code to your version config:
...
_†† See [Lobby environment variables](lobby-env) for more info._
...
## Custom accounts verification

If you plan on using your own game server instead of Rivet's identities, you can use the external
