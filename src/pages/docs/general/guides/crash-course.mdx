...
<Accordion title='What does this do?'>
  - Linked your project to Rivet - Creates `rivet.version.toml` to configure your deployed game. Read more
  about the Rivet version config [here](/general/concepts/version-config). - Creates a dev token added to your
  `.env` to tell Rivet to mock API endpoints in development. Read more about dev tokens
  [here](/general/concepts/dev-tokens).
</Accordion>
