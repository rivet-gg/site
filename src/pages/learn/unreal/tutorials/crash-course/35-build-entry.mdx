...
First, run the Rivet server:

<CodeGroup>
   <code title="Windows">
      ```powershell
      docker run -it --rm -v "${PWD}:/project" -p "127.0.0.1:7777:7777/udp" --entrypoint="/project/build_and_run_inner.sh" ghcr.io/epicgames/unreal-engine:dev-5.2.1
      ```
   </code>
