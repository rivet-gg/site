...
Godot provides great multiplayer capabilities out of the box. We'll be using Godot's built-in networking to
build a server-authoritative multiplayer game on top of Rivet.

Godot supports ENet, WebSocket, and WebRTC transports. We'll be using ENet for this tutorial, but it's easy
to swap out.
...
```
git clone https://github.com/rivet-gg/examples
```

And then navigate to the `godot/bomber` folder. The rest of this guide will focus on that server
...
Open the Rivet panel in the bottom left and click _Start Server_. Then click the play button in the top right
to start the client. You should be able to join the lobby.
