...
- [**User-Provided Config**](/docs/build/user-config) Modules can allow projects to pass configuration options to the module.
...
- `config.ts` _(optional)_ Defines user-provided config schema. [Documentation](/docs/build/user-config)
...
Make sure you've installed the Rivet CLI as described [here](/docs/quickstart).

---

### Prerequisites

- [Install Rivet CLI](/docs/quickstart)
- [Configure your IDE](/docs/build/ide)
...
<Accordion title='What did this do?'>
  This will create a `rivet.json` file in your directory with some default settings.
</Accordion>
...
<Tip>See documentation on the `module.json` config [here](/docs/build/module-config).</Tip>
...
```prisma {{"file": "modules/my_leaderboard/db/schema.prisma"}}
datasource db {
    provider = "postgresql"
...
<Tip>
  For more information about wwriting database schemas, see [Prisma's documentation on data
  modelling](https://www.prisma.io/docs/orm/prisma-schema/data-model).
</Tip>

<Info>
  Modules are powered by [PostgreSQL](https://www.postgresql.org/).

Learn about why we chose PostgreSQL [here](/docs/engine/design/postgres).
...
<Steps>
	<Step title="Create script">
    	...
    	<AccordionGroup>
    		<Accordion title="What did this do?">
    			...
    			```typescript {{"file": "modules/my_leaderboard/scripts/submit_score.ts"}}
    		...
    		<Accordion title="What's going on here?">
    			...
    			- `import { ScriptContext } from "../module.gen.ts";` imports the `ScriptContext`.
    				- The `module.gen.ts` file is generated by Rivet to provide a type-safe interface for the database, calling other modules, and reading the module config.
    			- `export interface Request {}` and `export interface Response {}` define the input and output of the script.
    				- Rivet will automatically generate strict schema validation for these types. This reduces unintentional errors and potential exploits/crashes.
    ...
    <Step title="Add dependencies & make public">
    	...
    	```json {{"file": "modules/my_leaderboard/module.json"}}
    	{
    		"dependencies": {
    			"users": {},
    			"rate_limit": {}
    ...
    <Step title="Throttle requests">
    	...
    	```typescript {{"file": "models/my_leaderboard/scripts/submit_score.ts"}}
    	export async function run(ctx: ScriptContext, req: Request): Promise<Response> {
...
<Tip>
  For more information about querying databases, see [Prisma's documentation on querying
  data](https://www.prisma.io/docs/orm/prisma-client/queries).
</Tip>
...
<Steps>
    ...
    <Step title="Update request & response">
    	...
    	```typescript {{"file": "modules/my_leaderboard/scripts/get_top_scores.ts"}}
    	export interface Request {
    		count: number;
    	}
    ...
    <Step title="Throttle requests">
    	...
    	```typescript {{"file": "models/my_leaderboard/scripts/get_top_scores.ts"}}
    	export async function run(ctx: ScriptContext, req: Request): Promise<Response> {
...
<Steps>
    ...
    <Step title="Success">
    	You've now written a full module from scratch. You can now [generate an SDK to use in your game](/docs/sdk) or [publish it to a registry](/docs/build/registry).
    </Step>
...
<Steps>
    ...
    <Step title="Write test">
    	...
    	```typescript {{"file": "modules/my_leaderboard/tests/e2e.ts"}}
