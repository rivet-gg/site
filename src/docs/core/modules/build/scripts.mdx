...
```typescript modules/lobbies/scripts/find.ts
...
```json modules/lobbies/module.json
"scripts": {
  "create": {
    "name": "Create Lobby",
...
await client.Lobbies.FindOrCreate(new Rivet.Model.Lobbies.FindOrCreateRequest(
```
</CodeGroup>
...
```typescript modules/users/scripts/fetch.ts
