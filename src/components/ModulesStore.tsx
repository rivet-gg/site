...
import { faPlus, faBellConcierge, faHammer } from '@fortawesome/sharp-solid-svg-icons';
import clsx from 'clsx';
import { LayoutGroup, motion } from 'framer-motion';
import Link from 'next/link';
import { useState } from 'react';

export function ModulesStore({ categories }) {
  ...
  const filteredCategories = categories
    .map(category => {
      const modules = category.modules.filter(
        ({ module }) =>
          module.config.name.toLowerCase().includes((query || '').toLowerCase()) ||
          module.config.description.toLowerCase().includes((query || '').toLowerCase())
      );
              ...
              <motion.section
                ...
                <div className='grid grid-cols-1 items-start gap-4 sm:grid-cols-3'>
                  {category.modules.map(({ id, module }) => (
                    <Link href={`/modules/${id}`} key={id} className='h-full'>
                      <ModuleCard {...module.config} />
                    </Link>
                  ))}
                </div>
              </motion.section>
