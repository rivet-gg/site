...
function PropertyTypeLabel(props: PropertyTypeLabelProps) {
    return (
        <span className="text-xs text-charcole-700">
            {getPropertyTypeLabel(props)}
        </span>
    );
}
...
function ObjectSchema({ schema, parent }: ObjectSchemaProps) {
    return (
        <div>
            <ul className="border border-charcole-900 px-4 py-2 rounded-md my-4">
                {Object.keys(schema.properties).map((key) => {
                    ...
                    const isFoldable = !isEmptyObject ||
                        (isArray ? !isEmptyObject : false);
...
function Content(
    { schema, parent = undefined }: ContentProps,
) {
    if (schema.type === "object") {
