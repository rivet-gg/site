...
export const RIVET = new RivetClient({
  token:
    'pub_low_resources.eyJ0eXAiOiJKV1QiLCJhbGciOiJFZERTQSJ9.CMaYo7CGMhDGwN7ykDEaEgoQB8GHsWkRRheihVf774gvqCIWGhQKEgoQJcVm7iAXTEmJTnGUyTIR6Q.DRJPgeMDfrUetWKypb5vyrDScz0kMN9DO_vXs_3l1L3zQbzD751IhBJtzfrOlQLY1JTolwhh5TueOKawZpCxCA'
});
...
function renderBackground(client: Client, ctx: CanvasRenderingContext2D) {
  if (client.assets.tileSand.complete) {
    const tileSize = client.assets.tileSand.height * client.assets.scaleFactor;
    const tileXMin = Math.floor((client.cameraOffsetX - client.screenWidth / 2) / tileSize);
    const tileXMax = Math.ceil((client.cameraOffsetX + client.screenWidth / 2) / tileSize);
    const tileYMin = Math.floor((client.cameraOffsetY - client.game.viewportHeight / 2) / tileSize);
    const tileYMax = Math.ceil((client.cameraOffsetY + client.game.viewportHeight / 2) / tileSize);
    for (let x = tileXMin; x <= tileXMax; x++) {
      for (let y = tileYMin; y <= tileYMax; y++) {
        ctx.drawImage(client.assets.tileSand, x * tileSize, y * tileSize, tileSize, tileSize);
      }
    }
  }
}
...
function renderWall(client: Client, ctx: CanvasRenderingContext2D) {
  if (client.assets.wall.complete) {
    ...
    for (let i = 0; i < wallCount; i++) {
      const progress = -paddedArenaSize / 2 + (i / (wallCount - 1)) * paddedArenaSize;
...
function renderMenu(client: Client, ctx: CanvasRenderingContext2D) {
  if (client.connection?.isDisconnected) {
    ...
    ctx.save();
    ctx.fillStyle = 'white';
    const instructions = ['Controls:', 'Aim: Mouse', 'Move: WASD', 'Fire: Space'];
    ctx.textAlign = 'left';
    ...
    for (let i = 0; i < instructions.length; i++) {
      ctx.fillText(
        ...
        client.screenHeight / 2 - 40 - (instructions.length - i - 1) * 50
      );
    }
    ctx.restore();
  }
...
function renderFullscreenMessage(client: Client, ctx: CanvasRenderingContext2D, message: string) {
  ctx.save();
  ctx.fillStyle = 'rgba(0, 0, 0, 0.25)';
  ctx.fillRect(-client.screenWidth / 2, -client.screenHeight / 2, client.screenWidth, client.screenHeight);
  ctx.restore();
