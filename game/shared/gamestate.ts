...
export const MAP_HEIGHT = Math.round(MAP_WIDTH / 896 * 400);
...
export function newRandomGame(): GameState {
  ...
  for (let i = 0; i < outputState.targetAsteroids; i++) {
    const newAsteroid = newRandomAsteroid([25, 60], [0, MAP_WIDTH], [0, MAP_HEIGHT], 100, [-2, 2]);
...
export function newBulletHellGame(asteroids: number = 200): GameState {
  const outputState: GameState = {
    asteroids: {},
    bullets: {},
    players: {},
  ...
  for (let i = 0; i < asteroids; i++) {
    const newAsteroid = newRandomAsteroid([10, 25], [0, MAP_WIDTH], [0, MAP_HEIGHT], 100, [-2, 2]);
    outputState.asteroids[newAsteroid.id] = newAsteroid;
  }
