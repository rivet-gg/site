...
export function updateAsteroid(asteroid: Asteroid, game: GameState, dt: number) {
  ...
  const pastLeftEdge = asteroid.posX - sizeAsteroid(asteroid) <= 0;
  const pastRightEdge = asteroid.posX + sizeAsteroid(asteroid) >= game.size.w;

  const pastTopEdge = asteroid.posY - sizeAsteroid(asteroid) <= 0;
  const pastBottomEdge = asteroid.posY + sizeAsteroid(asteroid) >= game.size.h;

  if (pastLeftEdge) asteroid.velX = Math.abs(asteroid.velX);
  if (pastRightEdge) asteroid.velX = -Math.abs(asteroid.velX);

  if (pastTopEdge) asteroid.velY = Math.abs(asteroid.velY);
  if (pastBottomEdge) asteroid.velY = -Math.abs(asteroid.velY);
