...
export function createPlayer(game: Game): PlayerState {
  const state = {
    id: generateId(game),
    positionX: Utilities.lerp(-game.arenaSize / 2, game.arenaSize / 2, Math.random()),
    positionY: Utilities.lerp(-game.arenaSize / 2, game.arenaSize / 2, Math.random()),
    aimDir: 0,
    moveX: 0,
    moveY: 0,
    health: 1,
...
export function updatePlayer(game: Game, state: PlayerState, dt: number) {
  ...
  // Restrain to bounds
  state.positionX = Math.max(state.positionX, -game.arenaSize / 2 + PLAYER_RADIUS);
  state.positionX = Math.min(state.positionX, game.arenaSize / 2 - PLAYER_RADIUS);
  state.positionY = Math.max(state.positionY, -game.arenaSize / 2 + PLAYER_RADIUS);
  state.positionY = Math.min(state.positionY, game.arenaSize / 2 - PLAYER_RADIUS);

  state.shootTimer -= dt;
}

export function renderPlayer(client: Client, state: PlayerState, ctx: CanvasRenderingContext2D) {
  ctx.save();
  ...
  const bodyHeight = client.assets.tankBodyRed.height * client.assets.scaleFactor;
  ...
  const barrelWidth = client.assets.tankBarrelRed.width * client.assets.scaleFactor;
  const barrelHeight = client.assets.tankBarrelRed.height * client.assets.scaleFactor;
  ctx.drawImage(
  ...
  ctx.fillStyle = 'white';
  ctx.fillRect(-healthWidth / 2, healthY - healthHeight / 2, healthWidth * state.health, healthHeight);
  ctx.restore();
...
export function damagePlayer(game: Game, state: PlayerState, amount: number, damagerId?: number) {
  state.health -= amount;
  if (state.health <= 0) {
