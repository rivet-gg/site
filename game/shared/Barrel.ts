...
export function createBarrel(game: Game, positionX: number, positionY: number): BarrelState {
  const state = {
    id: generateId(game),
    positionX: positionX,
    positionY: positionY,
...
export function renderBarrel(client: Client, state: BarrelState, ctx: CanvasRenderingContext2D) {
  ctx.save();
  ...
  ctx.drawImage(client.assets.barrel, -barrelWidth / 2, -barrelHeight / 2, barrelWidth, barrelHeight);
