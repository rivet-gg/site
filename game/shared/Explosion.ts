...
export function createExplosion(game: Game, positionX: number, positionY: number): ExplosionState {
  const state = {
    id: generateId(game),
    positionX: positionX,
    positionY: positionY,
...
export function renderExplosion(client: Client, state: ExplosionState, ctx: CanvasRenderingContext2D) {
  ctx.save();

  ctx.translate(state.positionX, -state.positionY);

  const explosionWidth = client.assets.explosion.width * client.assets.scaleFactor;
  const explosionHeight = client.assets.explosion.height * client.assets.scaleFactor;
  ctx.drawImage(
