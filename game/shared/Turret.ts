...
export function createTurret(game: Game, positionX: number, positionY: number): TurretState {
  const state = {
    id: generateId(game),
    positionX: positionX,
    positionY: positionY,
...
export function renderTurret(client: Client, state: TurretState, ctx: CanvasRenderingContext2D) {
  ctx.save();
  ...
  const bodyHeight = client.assets.turretBody.height * client.assets.scaleFactor;
  ctx.drawImage(client.assets.turretBody, -bodyWidth / 2, -bodyHeight / 2, bodyWidth, bodyHeight);
  ctx.restore();

  // Draw barrel
  ctx.save();
  ctx.rotate(state.aimDir - Math.PI / 2);
  const barrelWidth = client.assets.turretBarrel.width * client.assets.scaleFactor;
  const barrelHeight = client.assets.turretBarrel.height * client.assets.scaleFactor;
  ctx.drawImage(
...
function aimAtPlayer(game: Game, state: TurretState) {
  ...
  for (const playerId in game.state.players) {
    ...
    const dist = Math.sqrt(
      Math.pow(player.positionY - state.positionY, 2) + Math.pow(player.positionX - state.positionX, 2)
    );
